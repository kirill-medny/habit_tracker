...
def habit_post_save(sender, instance, created, **kwargs):
    ...
    user = instance.user
    chat_id = os.environ.get('TELEGRAM_CHAT_ID') # Получаем chat_id из переменной окружения
    ...
    send_telegram_message.delay(chat_id, message) # используем Celery для отправки сообщения